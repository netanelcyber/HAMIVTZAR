#!/usr/bin/env python3
"""
Command-line WAF Sensor Analysis Tool

Analyze obfuscated bot-detection scripts from URLs.

Usage:
    python3 analyze_sensor_url.py <url>
    python3 analyze_sensor_url.py https://target.com/sensor.js
    python3 analyze_sensor_url.py https://events-new.mepha.ch/_Incapsula_Resource?...
"""
...
class JSDeobfuscator:
    ...
    def decode_hex_escape_sequences(self) -> Dict[int, str]:
        r"""Extract and decode hex escape sequences (\x**)."""
...
def analyze_url(url: str) -> Dict[str, Any]:
    ...
    try:
        ...
        print("❌ Proxy Error: Unable to connect through proxy")
        print("   Note: This environment has proxy restrictions")
        print("   Try uploading the file to Colab instead")
        sys.exit(1)
    ...
    if results['string_arrays']:
        print("\n📋 Decoded Strings (first array, max 20 items):")
        first_array = results['string_arrays'][0].split('\n')[:20]
        for i, s in enumerate(first_array, 1):
            preview = s[:60] + "..." if len(s) > 60 else s
            print(f"  {i:2}. {preview}")
        total = len(results['string_arrays'][0].split('\n'))
        if total > 20:
            print(f"  ... ({total - 20} more items)")
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 analyze_sensor_url.py <url>")
        print("\nExamples:")
        print("  python3 analyze_sensor_url.py https://target.com/sensor.js")
        print("  python3 analyze_sensor_url.py 'https://events-new.mepha.ch/_Incapsula_Resource?SWJIYLWA=...'")
        print("\nNote: URL must be properly escaped/quoted if it contains special characters")
        sys.exit(1)
